---
id: vertical-lookup
title: Vertical Lookup (Vlookup)
---

import { AuthorCard } from "@site/src/components/author-card.js";

<AuthorCard
	author={[
		{
			name: "Zi Ching Goh",
			image: "/img/squad-profile-images/zi-ching-goh.jpg",
			linkedinUrl: "https://www.linkedin.com/in/zi-ching-g-750b26198/",
		},
		{
			name: "Tuan Sihan",
			image: "/img/squad-profile-images/tuan-sihan.png",
			linkedinUrl: "https://www.linkedin.com/in/tuansihan/",
		},
		{
			name: "Sher Ree Koh",
			image: "/img/squad-profile-images/sher-ree-koh.png",
			linkedinUrl: "https://www.linkedin.com/in/koh-sher-ree-b8a62a204/",
		},
		{
			name: "Ain Batrisyia",
			image: "/img/squad-profile-images/ain-batrisyia.jpeg",
			linkedinUrl: "https://www.linkedin.com/in/ainbatrisyiashaharuddin/",
		},
		{
			name: "Hira Khalique",
			image: "/img/squad-profile-images/hira-khalique.jpg",
			linkedinUrl: "https://www.linkedin.com/in/hira-khalique-9871211a3/",
		},
	]}
/>

## 😵‍💫 Feeling Dizzy to find what you want among many information?

Here are some tips that will help!

:::note

**V-lookup** stands for **"Vertical Lookup"**. It is a function that makes Excel search for a certain value in a column, in order to return a value from a different column in the same row.

:::

Let us begin!

### Step 1

Type in `=vlookup(` as shown below.

![Step 1](./step-1.png)

### Step 2

As the tooltip suggested, the **lookup_value** should be in cell `E2`.

![Step 2](./step-2.png)

### Step 3

As for the **table_array** in the tooltip, highlight the whole table that contains the missing information, which is `A1:C6`.

![Step 3](./step-3.png)

### Step 4

As for the **col_index_num** in this case, the **"ID number"** is in column 2.

![Step 4](./step-4.png)

### Step 5

Ensure **comma** is in place, and a **context box** will be showing up as shown below.

:::tips

-   **True** means that a "close" match will be returned.
-   **False** means that VLOOKUP is looking for an **EXACT** match.

:::

![Step 5](./step-5.png)

### Step 6

Lastly, as for the **range_lookup**, type in **false**, close the bracket, and press Enter key.

![Step 6](./step-6.png)

### Tada ~~~ We are done! 🎊

![final-result](./final-result.png)

## Stay connected with us

[Instagram](https://www.instagram.com/microsoft_mws/) |
[Facebook](https://www.facebook.com/microsoft.mws) |
[LinkedIn](https://www.linkedin.com/company/mwsquad/)
